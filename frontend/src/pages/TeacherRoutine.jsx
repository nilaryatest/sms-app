...
import { Loader2, Calendar, Clock, BookOpen, CalendarX2 } from 'lucide-react';
        ...
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="space-y-1">
                    <h1 className="text-4xl font-black tracking-tight text-gray-900">Teaching Routine</h1>
                    <p className="text-lg text-gray-500 font-medium">Your weekly class and lecture schedule.</p>
                </div>
            </header>
            ...
            {loading ? (
                ...
                <div className="p-16 text-center bg-white rounded-3xl border border-gray-100 shadow-sm">
                    ...
                    <p className="text-gray-400">You don't have any classes assigned to you currently.</p>
                </div>
            ) : (
                                        ...
                                        {dayRoutines.map((routine) => (
                                            <div key={routine.id} className="group flex flex-col gap-3 p-4 rounded-2xl bg-gray-50/80 border border-gray-100 hover:bg-white hover:border-indigo-100 hover:shadow-sm transition-all">
                                                ...
                                                <div className="flex items-center gap-4 text-sm font-medium text-gray-500">
                                                    ...
                                                    <div className="flex items-center gap-1.5 bg-white px-3 py-1.5 rounded-lg border border-gray-100">
                                                        <span className="font-bold text-gray-400 text-xs uppercase tracking-widest">Section {routine.section_id}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
